...
public class LeashDecorationModule implements Module, Listener {
    ...
    private final Map<UUID, Wolf> pendingLeash = new ConcurrentHashMap<>();
    ...
    public void enable() {
        ...
        swingTask = FoliaScheduler.runGlobalAtFixedRate(plugin, 1L, 1L, () -> {
            for (Map.Entry<UUID, Wolf> entry : pendingLeash.entrySet()) {
                ...
                FoliaScheduler.runEntity(plugin, anchor, () -> {
                    if (!anchor.isValid() || anchor.isDead()) {
                        return;
                    }
                    Location anchorLoc = anchor.getLocation();
                    FoliaScheduler.runEntity(plugin, player, () -> applySwingPhysics(player, anchorLoc));
                });
            }
        });
    }
    ...
    private void applySwingPhysics(Player player, Location anchorLoc) {
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (!ModuleAccess.canBuild(plugin, player, wolfLoc)) {
            cancelPending(player);
            return;
        }
    ...
    private void detachAndReattachToPlayer(Wolf wolf, Location clickedFence, Player player) {
        ...
        if (!ModuleAccess.canBuild(plugin, player, wolfBlock) || !ModuleAccess.canBuild(plugin, player, clickedBlock)) {
            return;
        }
        ...
        if (wolfBlock.equals(clickedBlock)) {
            if (wolf.isLeashed() && wolf.getLeashHolder() instanceof LeashHitch hitch) {
                ...
                if (!ModuleAccess.canBuild(plugin, player, otherFence)) {
                    return;
                }
